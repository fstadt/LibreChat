import { dataService, MutationOptions, QueryKeys, Tools } from 'librechat-data-provider';
...
export const useValidateToolCallMutation = (
  validationId: string,
  options?: MutationOptions<t.ToolCallResponse, never>,
): UseMutationResult<t.ToolCallResponse, Error> => {
  const queryClient = useQueryClient();
  return useMutation(
    (validationId: string) => {
      return dataService.validateToolCall({
        validationId,
      });
    },
    {
